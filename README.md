...
- **自然に回復するのは 2 つだけ**。レベルアップ（最大 HP の 50%）と、Claude が実行したテストの成功（35%）。あとは**マップでショップを見つけたときだけ**、所持金で薬や装備を買える。
...
- **ダッシュボード**では、このセッションでプラグインが受け取ったイベント、Claude のトークン消費、カニード の進行をグラフで見られる。
...
| ステータス画面（`/kaneed status`） |
|---|
| ![ステータス画面: 攻撃・防御・会心・回避と 5 部位の装備](docs/status.png) |
...
このセッションの様子を 3 つのグラフ群で表示する。帯が広ければ横に 3 列、狭ければ縦に積む。集計はメモリ上だけで、セッションが変わると 0 から始まる。

ダッシュボードもトークンを消費しない。数字はすべて、プラグインが受け取るイベントの入力（`turn.complete` に付いてくる、Claude の応答で実際に使われた usage など）と、ステータスラインと同じ `$.session.usage()`（引数なしの呼び出しは API リクエストを出さない）から取る。モデル呼び出し・サブエージェント・プロンプト送信・圧縮・内訳付き usage を `hooks/` で使っていないことは `tests/no-tokens.test.ts` が確かめる。

| 区画 | 内容 |
|---|---|
| トークン | ターンごとの 出力 / 入力 / Cache 読込 / Cache 作成 のスパークライン（1 本 = 1 ターン）と合計、セッションのコスト（USD）とコンテキスト使用率（`turn.complete` の usage と `$.session.usage()`） |
| イベント | 毎分のイベント数の推移と、プロンプト・ターン開始/完了・サブエージェントのターン・ツール呼び出し（ツール別内訳）・スキル・圧縮・テスト成功/失敗 の件数の横棒グラフ |
| ゲーム進行 | セッション中の HP（割合）・Lv・階層・討伐数（死亡をまたいで累計）・所持金 の推移と増分、歩数・Lv アップ・装備入手・死亡の回数 |
...
- **ショップ**は半分くらいの階にある（入口の部屋と階段の部屋以外。ミニマップの `S`）。見つけると カニード は寄り道して入り、棚に並んだ 3 品のうち**所持金で買えるものからランダムに 1 つ**買う。店長が迎え、カーソルが棚を回って止まり、選んだ品が カニード に飛んでいって効く。1 階につき 1 回で、入ったあとは閉店（`s`）。

  | 品物 | 効果 | 値段 |
  |---|---|---|
  | 薬草 | HP を最大 HP の 30% 回復 | 15 + 5 × Lv G |
  | 回復薬 | HP を全回復 | 50 + 15 × Lv G |
  | 装備 | すぐ持ち替える（拾った装備と同じく弱体化もありうる）。段階は拾える装備より少し上 | 20 × (段階 × レア度倍率)^1.5 G（T1 コモン 20 G 〜 T5 レジェンド 約 730 G） |

  棚は 薬草 35%・回復薬 20%・装備 45% で埋まる。HP が満タンのときは薬を買わない。買えるものが無ければ何も買わずに出る。倒れると所持金も 0 に戻る。

  ![ショップ: 店長が迎え、カーソルが棚を回って止まり、選んだ品が カニード に飛んでいく](docs/shop.gif)
- **死亡**すると 4 秒後に冒険番号が 1 つ進み、Lv 1 から再開。

バランスの目安（シミュレーション 30 回の中央値、テスト成功なし）は 1 回の冒険で約 16 分。ショップに寄るのは 1 回の冒険で平均 1.4 回ほど。45 秒ごとにテストが通ると約 27 分。序盤の運が悪いと 1 分以内に倒れることもある。
