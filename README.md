...
| `設定` ボタン / `/kaneed settings` | 帯の高さ、キャラクターの大きさ、ミニマップ、主人公を選ぶ（下記） |
...
| キャラクターの大きさ | 自動（帯の高さいっぱい）/ ×1 / ×2 / ×3 | `/kaneed set size 2` |
| ミニマップ | 表示 / 非表示 | `/kaneed set map off` |
| 主人公 | カニード（朱・藍・翠・紫・金）/ フォールバック像 | `/kaneed set hero ai` |
| 初期値に戻す | — | `/kaneed set reset` |

キャラクターは帯の高さに合わせて拡大される。ステージに立つもの（主人公・敵・宝箱・階段）はすべて同じ倍率で描かれ、装備もその倍率で主人公に乗る。整数倍が入らない帯では、入るだけ拡大する。設定画面の最後の行に、その端末で実際に使われている倍率が出る。
