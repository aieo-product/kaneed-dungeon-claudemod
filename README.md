...
主人公は差し替え前提で作ってある。24×16 のドット絵を 1 枚描いて置くだけで、既存の装備 25 枚（帽子・アイウェア・盾・剣・足装備 × 5 段階）がそのまま乗る。設定画面で選べる色違いとフォールバック像も、この仕組みの上に乗っている。
...
| `設定` ボタン / `/kaneed settings` | 帯の高さ、キャラクターの大きさ（縮小）、ミニマップ、主人公を選ぶ（下記） |
...
設定画面のボタンは、押すたびに次の選択肢へ進む。キーボードからは `/kaneed set <項目> <値>` でも指定できる。選んだ内容は保存され、次のセッションでも同じになる。

| 項目 | 選択肢 | コマンド |
|---|---|---|
| 帯の高さ | 自動 / 12 / 16 / 20 / 22 行 | `/kaneed set rows 16` |
| キャラクターの大きさ | 自動 / そのまま / ×0.75 / ×0.5 | `/kaneed set size 0.5` |
| ミニマップ | 表示 / 非表示 | `/kaneed set map off` |
| 主人公 | カニード（朱・藍・翠・紫・金）/ フォールバック像 | `/kaneed set hero ai` |
| 初期値に戻す | — | `/kaneed set reset` |

![キャラクターの大きさ: そのまま / ×0.75 / ×0.5 の比較](docs/sizes.png)

キャラクターはドット絵のままの大きさが最大で、それ以上には拡大しない。小さくしたいときに縮める。ステージに立つもの（主人公・敵・宝箱・階段）はすべて同じ倍率で描かれ、装備もその倍率で主人公に乗る。帯が低くて指定した大きさが入らないときは、入るところまで自動で縮む。設定画面の最後の行に、その端末で実際に使われている大きさが出る。

色違いの主人公は、描かれた絵の色相をその場でずらして作っている（白い目と黒い口は無彩色なので変わらない）。
...
| ゲーム記録 | ステータス画面に出ない数字だけ。死亡・最高到達階・最高 Lv・歩数、撃破数（ボス内訳）、攻撃・会心・与ダメージ、被弾・回避・被ダメージ、回復回数（テスト成功 / Lv アップ / ショップの薬）、宝箱・装備入手（弱体化した回数）・獲得 G と買い物（使った G）の累計。死をまたいで積み上がる |
